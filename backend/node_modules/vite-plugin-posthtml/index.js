...
const posthtmlPlugin = (opts = {}) => {
  ...
  const observer = new PerformanceObserver(items => {
    ...
    if (total?.duration > 250) {
      console.error(`PostHTML took ${Math.round(total.duration)}ms to run for path ${total.name.split(':')[1]}`);
    }
  ...
  return {
    ...
    async transformIndexHtml(input, { path: pth }) {
      ...
      performance.measure(`vite-plugin-posthtml-duration:${pth}`, `vite-plugin-posthtml-start:${pth}`);
      return html;
    },
  };
};
